...
button {
	...
	transition: background-color 0.3s ease, color 0.3s ease; /* 배경색 및 글자색 변경 시 애니메이션 */
}
</style>
</head>
<body>
    ...
    function calculatePercent() {
        var ratios = Array.from(document.querySelectorAll('.ratio')).map(input => parseFloat(input.value) || 0);
        var totalRatio = ratios.reduce((acc, ratio) => acc + ratio, 0);

        var percentInputs = document.querySelectorAll('.percent');
        for (var i = 0; i < percentInputs.length; i++) {
            var ratio = ratios[i];
            var percent = (totalRatio > 0) ? (ratio / totalRatio * 100).toFixed(2) + '%' : '0%';
    ...
    document.getElementById('addRow').addEventListener('click', function() {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            if (i === 1) {
                ...
                input.onchange = calculatePercent;
            } else if (i === 2) {
                input.type = 'text';
